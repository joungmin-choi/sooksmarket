...
app.post('/sm_unsign', function(req, res){
  var str1 = 'SELECT * FROM TradeInfo WHERE (seller=? AND state<4) OR (customer=? AND state<4)';
  var flag = 0;

  async.series([
    function(callback){
      client.query(str1, [loginId[1], loginId[1]], function(err, result){
        //console.log(result);//flag = 1;
  ...
  function(err){
    if(flag === 1){
      ...
      //console.log("0");
      // var string = '<script type="text/javascript">alert("거래가 완료되어야 탈퇴하실 수 있습니다.\n[mypage]->[나의거래내역]에서 확인하세요.");</script>';
      // res.send(string);
    }
  });
});
...
app.post('/test', multipartMiddleware, function(req, res){
  //console.log("hello:",req.files.file);
  var file = [];
  var name = [];
  var path = [];
  var type = [];
  ...
  var sql = 'INSERT INTO test (photo1, photo2, photo3) VALUES (?,?,?)';
  client.query(sql, [outputPath[0], outputPath[1], outputPath[2]], function() {
  });
});
